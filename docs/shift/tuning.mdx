...
Ride Height - Stage 2: You want to use the lowest settings that avoid bottoming out frequently. Smoother tracks with smaller curbs and low downforce setups allow lower ride height to be used. High downforce setups on high speed tracks might require increased ride height, especially if increasing spring stiffness is not an option. High power cars squat significantly while accelerating, requiring 15mm higher rear ride height to keep the car mostly level. Positive rake (high rear, low front) usually doesn't work well if the car generates significant downforce - it's better to focus on keeping the car level. Negative rake (high front, low rear) is not recommended.

### Springs and Dampers

Sway Bars - Always available: The main way to adjust balance between oversteer and understeer. Stiffer front is more understeer, stiffer rear is more oversteer and vice versa. If set too soft, the car might become unstable during cornering, especially at medium and high speed. At least 3 steps stiffer front and rear are recommended for most cars. Soften by at most one step or harden by 1-3 steps to balance the car. Bumpy tracks like the Nordschleife can benefit from softer sway bars to reduce mid corner bouncing.
...
3. Start balancing the car by adjusting [sway bars](#springs-and-dampers). Either start from default or 3 steps right for both. Switch to adjuting slow bump by one or two steps if you need finer control.
	- You can attempt adjusting balance with spring rate too, but I found the results unsatisfactory. Keep in mind you can also adjust balance in mid-high speed corners by changing downforce. Change the ride height if needed.
